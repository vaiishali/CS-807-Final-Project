...
const int greenPin = 5; // ~D6
const int bluePin = 4; // ~D5
...
//   WiFi.config(WebServerIP, Gatway, Subnet)
  ...
  WiFi.config(IPAddress(192, 168, 1, 221), IPAddress(192, 168, 43, 1), IPAddress(255, 255, 255, 0));
...
void loop() {
  ...
  analogValue = analogRead(0); // read the analog signal
  //analogValue = 1;
  delay(100);
